class_name EffectSystemEnhanced
extends RefCounted

## Enhanced effect system with resistance, immunities, and better mechanics

const StatusEnhanced = preload("res://battle/models/StatusEnhanced.gd")
...
func _try_apply_single_effect(effect_data: Dictionary, caster: Unit, target: Unit) -> Dictionary:
	...
	var new_status := StatusEnhanced.new(status_id, status_name, final_duration, null, metadata)
...
func _handle_stacking(target: Unit, existing: StatusEnhanced, new_status: StatusEnhanced) -> Dictionary:
	match existing.stack_type:
		StatusEnhanced.StackType.NONE:
			return {
				"success": false,
				"reason": "no_stack",
		...
		StatusEnhanced.StackType.STACK:
		...
		StatusEnhanced.StackType.REFRESH:
		...
		StatusEnhanced.StackType.INTENSITY:
		...
		StatusEnhanced.StackType.EXTEND:
			existing.stack_with(new_status)
			return {
				"success": true,
...
func process_end_of_round_effects(units: Array[Unit]) -> Array[String]:
	...
	for unit in units:
		...
		var expired_statuses: Array[StatusEnhanced] = unit.tick_statuses()
...
func _process_single_status(status: StatusEnhanced, unit: Unit) -> Array[String]:
	var logs: Array[String] = []
	
	match status.category:
		StatusEnhanced.Category.DOT:
		...
		StatusEnhanced.Category.BUFF:
			# Buffs are typically handled elsewhere (stat calculations)
			pass
		
		StatusEnhanced.Category.DEBUFF:
...
func _sort_by_priority(a: StatusEnhanced, b: StatusEnhanced) -> bool:
	return a.priority < b.priority

## Get application message based on effect type
func _get_application_message(target: Unit, status: StatusEnhanced) -> String:
	match status.category:
		StatusEnhanced.Category.DOT:
			return "%s is afflicted with %s!" % [target.name, status.name]
		StatusEnhanced.Category.CONTROL:
...
func _get_dot_message(unit: Unit, status: StatusEnhanced, damage: int) -> String:
	match status.id:
		"poison":
...
func _get_hot_message(unit: Unit, status: StatusEnhanced, healing: int) -> String:
	match status.id:
		"regen":
...
func _get_expiration_message(unit: Unit, status: StatusEnhanced) -> String:
...
func _track_effect_applied(caster: Unit, target: Unit, status: StatusEnhanced) -> void:
...
func _track_effect_expired(unit: Unit, status: StatusEnhanced) -> void:
...
func cleanse_effects(target: Unit, categories: Array[StatusEnhanced.Category]) -> Array[String]:
	var logs: Array[String] = []
	var removed: Array[String] = []
	
	target.statuses = target.statuses.filter(func(s: StatusEnhanced):
...
func get_effects_by_category(unit: Unit, category: StatusEnhanced.Category) -> Array[StatusEnhanced]:
	return unit.statuses.filter(func(s: StatusEnhanced):
