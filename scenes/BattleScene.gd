...
func _on_menu_action(kind: String, id: String) -> void:
	if battle_finished:
		return
	match kind:
		"attack": _queue_hero_action(skill_slash)
		"spells":
			if id == "fireball": _on_fireball() else: _log("Unknown spell: %s" % id)
		"items":
			if id == "potion": _on_potion() else: _log("Unknown item: %s" % id)
		"defend":
			_log("%s braces for impact (Defend)." % hero.name)
			planned_actions.clear(); _refresh_plan_label(); _refresh_end_turn_button(); return
	_refresh_end_turn_button()

func _on_attack() -> void:
	if !battle_finished: _queue_hero_action(skill_slash)

func _on_fireball() -> void:
	if battle_finished: return
	var cost := int(skill_fireball.get("mp_cost", 0))
	if int(hero.stats.get("MP",0)) >= cost: _queue_hero_action(skill_fireball) else: _log("Not enough MP for Fireball!")

func _on_potion() -> void:
	if battle_finished: return
	if potion_used: _log("The potion bottle is empty."); return
	var cur := int(hero.stats.get("HP",0)); var max := int(hero.max_stats.get("HP",cur))
	if cur >= max: _log("HP is already full!"); return
	var healed := hero.heal(int(ceil(max * POTION_HEAL_PCT)))
	potion_used = true; _log("Hero uses Potion and heals %d HP." % healed)
	_update_ui(); _refresh_end_turn_button(); _show_command_menu()
...
func _queue_hero_action(skill: Dictionary) -> void:
	...
	_refresh_plan_label(); _refresh_end_turn_button()

func _on_end_turn() -> void:
	if battle_finished or !hero.is_alive() or !enemy.is_alive(): return
	if planned_actions.is_empty(): planned_actions.append(Action.new(hero, skill_slash, enemy))
	_refresh_plan_label(); _refresh_end_turn_button()
	var enemy_action := Action.new(enemy, skill_slash.duplicate(true), hero)
	var actions: Array = planned_actions.duplicate(); actions.append(enemy_action)
	actions = turn_engine.build_queue(actions); _update_turn_order(actions)
	for a in actions:
		if a.actor == hero:
			var mp := int(a.skill.get("mp_cost",0)); if mp>0: hero.spend_mp(mp)
	for a in actions:
		if a.actor==null or a.target==null or !a.actor.is_alive(): continue
		...
		await _play_attack_animation(a, res); _update_ui()
	for line in turn_engine.end_of_round_tick([hero, enemy]): _log(line)
	planned_actions.clear(); _check_end(); _update_ui(); _update_turn_order([]); _refresh_plan_label(); _refresh_end_turn_button()
	if !battle_finished: _show_command_menu()

func _unhandled_input(e: InputEvent) -> void:
	if battle_finished: return
	if e.is_action_pressed("ui_action_1"): _on_attack()
	elif e.is_action_pressed("ui_action_2"): _on_fireball()
	elif e.is_action_pressed("ui_action_3"): _on_potion()
	elif e.is_action_pressed("ui_action_4") and keyboard_end_turn_enabled: _on_end_turn()

func _check_end() -> void:
	if battle_finished: return
	if !enemy.is_alive(): _log("Victory! %s is defeated." % enemy.name); show_battle_result(true,0,[])
	elif !hero.is_alive(): _log("Defeat... The hero falls."); show_battle_result(false)

func _disable_inputs() -> void:
	btn_attack.disabled = true; btn_fire.disabled = true; btn_potion.disabled = true; btn_end.disabled = true; battle_finished = true
...
func _update_ui() -> void:
	if lbl_hero and hero:
		...
		if hero_hp_bar: hero_hp_bar.max_value = hero.max_stats.get("HP",0); hero_hp_bar.value = hero.stats.get("HP",0)
		if hero_mp_bar: hero_mp_bar.max_value = hero.max_stats.get("MP",0); hero_mp_bar.value = hero.stats.get("MP",0)
	if lbl_enemy and enemy:
		lbl_enemy.text = "%s\nHP %d/%d" % [enemy.name, enemy.stats.get("HP",0), enemy.max_stats.get("HP",0)]
		if enemy_hp_bar: enemy_hp_bar.max_value = enemy.max_stats.get("HP",0); enemy_hp_bar.value = enemy.stats.get("HP",0)
	if hero_portrait_rect and hero: hero_portrait_rect.texture = PortraitLoader.get_portrait_for(hero.name)
	if enemy_portrait_rect and enemy: enemy_portrait_rect.texture = PortraitLoader.get_portrait_for(enemy.name)
	_update_sprites(); refresh_status_hud(); _refresh_plan_label(); _refresh_end_turn_button()

func _log(msg: String, color: Color = Color(1,1,1), rich := false) -> void:
	var line := msg
	if !rich and color != Color(1,1,1): line = "[color=%s]%s[/color]" % [color.to_html(false), msg]
	log_view.append_text(line+"\n"); log_view.scroll_following = true; print(msg)

func _update_turn_order(actions: Array) -> void:
	if actions.is_empty(): lbl_queue.text = "Turn order: --"; return
	var parts: Array[String] = []
	for item in actions:
		var act := item as Action; if act==null: continue
...
func _fetch_skill(id: String) -> Dictionary:
	if DataRegistry.skills.has(id): return DataRegistry.skills[id].duplicate(true)
...
func _build_unit_from_character(id: String) -> Unit:
	...
	if def.is_empty(): def = {"name":"Pyro Adept","stats":{"max_hp":90,"max_mp":40,"atk":10,"def":8,"agi":12,"focus":16},"resist":{"fire":0.5,"water":1.5,"earth":1.0,"air":1.0}}
	return _build_unit(def)

func _build_unit_from_enemy(id: String) -> Unit:
	var def := DataRegistry.enemies.get(id, {})
	if def.is_empty(): def = {"name":"Goblin","stats":{"max_hp":70,"max_mp":0,"atk":12,"def":6,"agi":10,"focus":6},"resist":{"fire":1.0,"water":1.0,"earth":1.0,"air":1.0}}
	return _build_unit(def)
...
func _build_unit(def: Dictionary) -> Unit:
	...
	var s := def.get("stats", {})
	var max_hp := int(s.get("max_hp",80)); var max_mp := int(s.get("max_mp",0))
...
func _swap_for_animated_sprite(old_sprite: Sprite2D, character: String, facing_back: bool) -> AnimatedFrames:
	if old_sprite == null: return null
	var parent := old_sprite.get_parent(); var idx := -1
	if parent: idx = parent.get_children().find(old_sprite)
	var a := AnimatedFrames.new()
	a.centered = old_sprite.centered; a.position = old_sprite.position; a.scale = old_sprite.scale; a.z_index = old_sprite.z_index; a.flip_h = old_sprite.flip_h
	a.character = character; a.set_facing_back(facing_back)
	if parent: parent.add_child(a); if idx>=0: parent.move_child(a, idx)
	old_sprite.queue_free(); return a
...
func _shadow_color_for(u: Unit) -> Color:
	if u==null: return Color(0,0,0,0.2)
	return Color(0,0,0,0.45) if u.is_alive() else Color(0,0,0,0.2)

func _play_attack_animation(a: Action, res: Dictionary) -> void:
	var s := _sprite_for_unit(a.actor); if s==null: return
	var sh := _shadow_for_unit(a.actor); var o := _origin_for_unit(a.actor); var sb := _shadow_base_scale(a.actor)
	...
	if sh: t.parallel().tween_property(sh, "scale", sb*Vector2(1.2,0.75), 0.12)
	t.tween_property(s, "position", o, 0.16).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	if sh: t.parallel().tween_property(sh, "scale", sb, 0.16)
	await t.finished
	if res.get("hit",false) and res.get("damage",0) > 0: await _shake_sprite(a.target)
	if res.get("hit",false): await _flash_sprite(a.target)

func _flash_sprite(u: Unit) -> void:
	var s := _sprite_for_unit(u); if s==null: return
	var base := _base_modulate_for(u); var t := create_tween()
...
func _shake_sprite(u: Unit) -> void:
	var s := _sprite_for_unit(u); if s==null: return
	var o := _origin_for_unit(u); var off := Vector2(12,0); if u==enemy: off.x = -off.x
	var t := create_tween(); t.tween_property(s, "position", o+off, 0.05); t.tween_property(s, "position", o-off*0.6, 0.07); t.tween_property(s, "position", o, 0.08); await t.finished

func _refresh_plan_label() -> void:
	if plan_label==null: return
...
func _refresh_end_turn_button() -> void:
	if btn_end: btn_end.disabled = battle_finished or planned_actions.is_empty()
...
func _populate_status_container(container: HBoxContainer, unit: Unit) -> void:
	...
	for name in unit.get_status_types():
		...
		seen[key] = true
		var r := TextureRect.new(); r.texture = _get_status_icon(key); r.size = Vector2(18,18); r.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
		container.add_child(r)

func _get_status_icon(status_name: String) -> Texture2D:
	var key := status_name.to_lower()
	if !status_icon_cache.has(key): status_icon_cache[key] = SpriteFactory.make_status_icon(key)
	return status_icon_cache[key]

func spawn_damage_popup(node: Node2D, amount: int, crit:=false, miss:=false) -> void:
	if node==null or popups_container==null or fx_controller==null: return
...
func play_sfx(kind: String) -> void:
	...
	if s==null: s = sfx_streams.get("hit", null) as AudioStream
	if s==null: return
	var pl: AudioStreamPlayer = $SFX; pl.stream = s; pl.play()

func _make_tone(freq: float, duration: float, volume: float = 0.35) -> AudioStreamWAV:
	var w := AudioStreamWAV.new(); w.format = AudioStreamWAV.FORMAT_16_BITS; w.mix_rate = 44100; w.stereo = false
	var n := max(1, int(duration*w.mix_rate)); var data := PackedByteArray(); data.resize(n*2)
	for i in range(n):
		var t := float(i)/w.mix_rate; var fi := min(1.0, t/0.02); var fo := min(1.0, (duration-t)/0.06); var env := min(fi, fo)
		var v := int(sin(TAU*freq*t)*volume*env*32767.0); v = clamp(v, -32768, 32767)
		data[i*2] = v & 0xFF; data[i*2+1] = (v>>8) & 0xFF
	w.data = data; w.loop_mode = AudioStreamWAV.LOOP_DISABLED; return w
