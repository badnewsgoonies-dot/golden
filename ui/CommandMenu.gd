...
func _build_if_needed() -> void:
    ...
    _btn_attack = _make_btn("Attack", func(): _emit_main("attack"))
    _btn_spells = _make_btn("Spells", func(): _toggle_submenu("spells"))
    _btn_items = _make_btn("Items", func(): _toggle_submenu("items"))
    _btn_defend = _make_btn("Defend", func(): _emit_main("defend"))
...
func _draw_bracket() -> void:
    ...
    var h := size.y
    var c := Color(1, 1, 1, 1)
    draw_polyline(PackedVector2Array([Vector2(2, 2), Vector2(w - 8, h / 2.0), Vector2(2, h - 2)]), c, 3.0)
    draw_polyline(PackedVector2Array([Vector2(10, 8), Vector2(w - 12, h / 2.0), Vector2(10, h - 8)]), c, 3.0)

func _draw_cursor() -> void:
    var w := 16.0
    var h := max(20.0, size.y)
    var mid := h * 0.5
    var c := Color(1, 1, 1, 1)
    ...
    draw_polygon(PackedVector2Array([Vector2(5, 3), Vector2(w - 3, mid), Vector2(5, h - 3)]), PackedColorArray([notch_col]))

func _move_cursor_to(btn: Control) -> void:
    if btn == null:
        return
    var top_left: Vector2 = _panel.to_local(btn.get_global_position())
    var y := top_left.y + (btn.size.y - _cursor.custom_minimum_size.y) * 0.5
    var x := 6.0
...
func _grab_focus(idx: int) -> void:
    ...
    _move_cursor_to(btns[_focused_idx])
