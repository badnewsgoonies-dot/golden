...
func _unhandled_input(e: InputEvent) -> void:
	...
	if e.is_action_pressed("ui_down"):
		...
		print("CommandMenu: ui_accept pressed, triggering button at index ", _cursor_idx)
...
func _emit_main(kind: String, id: String) -> void:
	print("CommandMenu emitting signal: kind=", kind, " id=", id)
...
func _open_submenu(kind: String) -> void:
	...
	for entry in list:
		...
		var btn := Button.new()
		btn.text = label if mp == null else "%s  (%d MP)" % [label, int(mp)]
